...
% Last Modified by GUIDE v2.5 03-May-2015 16:50:34
...
%         guidata(hObject, handles);
        ...
        if (strcmp(imgInfo.ColorType, 'truecolor') == 1)
            [shapeFeatures image]=extractShapeFeatures(queryImage);
            ...
            textureFeatures=greyLevelCoMatrix(img);
            ...
            queryImageFeature = [hsvHist autoCorrelogram color_moments textureFeatures shapeFeatures  meanAmplitude msEnergy  wavelet_moments str2num(name)];
...
if ( ~isempty( jpg_files ) || ~isempty( png_files ) || ~isempty( bmp_files ) )
    ...
    for k = 1:totalImages
        ...
        elseif (strcmp(imgInfo.ColorType, 'truecolor') == 1)
            [shapeFeatures I]=extractShapeFeatures(image);
            hsvHist = hsvHistogram(I);
            autoCorrelogram = colorAutoCorrelogram(I);
            color_moments = colorMoments(I);
           ...
           textureFeatures=greyLevelCoMatrix(img);
            ...
            wavelet_moments = waveletTransform(image, imgInfo.ColorType);
            % construct the dataset
            set = [hsvHist autoCorrelogram color_moments  textureFeatures shapeFeatures meanAmplitude msEnergy wavelet_moments];
           
            fprintf(fileID,'%s .jpg-> %s %s %s %s %s\n',name,color_moments,  textureFeatures ,shapeFeatures ,meanAmplitude, msEnergy );
...
% --- Executes on button press in ExecuteSvm.
function ExecuteSvm_Callback(hObject, eventdata, handles)
% hObject    handle to ExecuteSvm (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% check for image query
if (~isfield(handles, 'queryImageFeature'))
...
[~, ~, cmat,flowerClass] = svm(numOfReturnedImgs, handles.imageDataset.dataset, handles.queryImageFeature, metric, handles.folder_name, handles.img_ext);

% plot confusion matrix
opt = confMatPlot('defaultOpt');
opt.className = {
    'Bus', 'Dino', 'Rose', ...
    'Deffodil','Butter Cup', 'Daisy', 'Dandelion', 'Iris', 'Sunflower','Windflower','Pansy','fritilary' 
    };
set(handles.flowerClass,'String',opt.className{flowerClass});
opt.mode = 'both';
